...
/** Turns the score's four sub-results into the modal's "why do I have
 ...
 *  that ratio is), and separately contributes a specific "{category}
 *  secured" completed-bullet (if any category is booked) plus up to 2
 *  `improve` bullets naming specific still-unbooked categories —
 *  `unbookedCategoryLabels`/`bookedCriticalCategoryLabel` are expected
 *  pre-sorted by priority (critical categories first) by the caller,
 *  which already has that context. */
export function getHealthScoreSummary(
  result: WeddingHealthResult,
  details: { unbookedCategoryLabels: string[]; bookedCriticalCategoryLabel?: string | null }
): HealthScoreSummary {
  const completed: string[] = [];
  const improve: string[] = [];

  if (details.bookedCriticalCategoryLabel) completed.push(`${details.bookedCriticalCategoryLabel} secured`);
