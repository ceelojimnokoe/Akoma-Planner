...
//
// Pro accommodation finder: lodging for out-of-town family, sorted by
// distance from the venue (nearVendorId links each listing back to a
// seeded venue — see prisma/schema.prisma). Accommodation is global
// reference data with no per-wedding "side" assignment field, so "by
// side" is handled as a workflow — the couple filters/sorts this same
// shared list and decides what to share with each side — rather than a
// persisted per-listing tag. See LEARNING.md for the reasoning.
...
export default async function AccommodationPage({
  ...
  const gate = requirePro(weddingPlan!.plan, "The accommodation finder");
  const city = (cityParam as City) ?? weddingPlan!.city;

  const accommodations = gate.allowed
